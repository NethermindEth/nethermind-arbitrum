...
- **Block-production semaphore never waits.** "CreateBlock mutex held" errors mean Nitro is calling `startSequencing` while a previous block is still being assembled. In the sequencer role this typically points at slow EVM execution or a stalled Stylus FFI call. See [troubleshooting](../troubleshooting.md#create-block-mutex-held).
