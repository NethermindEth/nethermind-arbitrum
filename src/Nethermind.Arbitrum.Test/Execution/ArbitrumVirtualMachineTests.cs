...
public class ArbitrumVirtualMachineTests
{
    ...
    [Test]
    public void PrecompileExecution_ThrowsOutOfGasButArbosVersionGreaterOrEqualToEleven_Reverts()
    {
        ArbitrumRpcTestBlockchain chain = ArbitrumRpcTestBlockchain.CreateDefault(builder =>
        {
            builder.AddScoped(new ArbitrumTestBlockchainBase.Configuration
        ...
        Address sender = TestItem.AddressA;
        long intrinsicGas = GasCostOf.Transaction + 64;
        // Enough to get to getL1FeesAvailable() but will throw in there when trying to read state
        long gasLimit = intrinsicGas + (long)ArbosStorage.StorageReadCost;
        ...
        long gasSpent = gasLimit; // because out of gas
        tracer.GasSpent.Should().Be(gasSpent);
        tracer.ReturnValue.Should().BeEmpty();
    ...
    [Test]
    public void PrecompileExecution_ThrowsOutOfGasAndArbosVersionLessThanEleven_FailsWithOutOfGas()
    {
        ArbitrumRpcTestBlockchain chain = ArbitrumRpcTestBlockchain.CreateDefault(builder =>
        {
            builder.AddScoped(new ArbitrumTestBlockchainBase.Configuration
        ...
        long gasSpent = gasLimit; // Because fail (no refund, and anyway ran out of gas)
        tracer.GasSpent.Should().Be(gasSpent);
        tracer.ReturnValue.Should().BeEmpty();
    ...
    [Test]
    public async Task ExecuteStylusDirectCall_ActivationIsExpired_FailsAndConsumesAllGas()
    {
        ArbitrumRpcTestBlockchain chain = new ArbitrumTestBlockchainBuilder()
        ...
        chain.PrefundAccount(sender, 1000.Ether()).Should().RequestSucceed("Prefunding sender account should succeed");

        // Deploy Stylus contract
        ResultWrapper<MessageResult> deployResult = chain.DeployStylusContract(sender, "Arbos/Stylus/Resources/counter-contract.wat",
            out byte[] wasmCode, out Address contractAddress);

        deployResult.Should().RequestSucceed();
        chain.LatestReceipts()[1].StatusCode.Should().Be(StatusCode.Success);

        // Activate Stylus contract
        ResultWrapper<MessageResult> activateResult = chain.ActivateStylusContract(sender, contractAddress);
        ...
        ResultWrapper<MessageResult> incrementResult = await chain.Digest(new TestL2Transactions(chain.InitialL1BaseFee, sender, incrementTx));

        // Increment call should fail as activated contract is expired (activatedAtHours was set to 1 hour since genesis)
        incrementResult.Result.Should().Be(Result.Success);
        TxReceipt incrementReceipt = chain.LatestReceipts()[1];
        incrementReceipt.StatusCode.Should().Be(StatusCode.Failure);
