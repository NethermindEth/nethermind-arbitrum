...
public class ArbitrumFullSimulationRunTests : ArbitrumRpcModuleTests
{
    ...
    public async Task FullRpcSequenceFromLog_ExecutesAllCallsSuccessfully()
    {
        ...
        ResultWrapper<MessageResult> resultAtMessageIndexResult = await chain.ArbitrumRpcModule.ResultAtMessageIndex(7UL); // Last processed message
        resultAtMessageIndexResult.Result.ResultType.Should().Be(ResultType.Success);
        resultAtMessageIndexResult.Data.BlockHash.Should().NotBeNull();
        resultAtMessageIndexResult.Data.SendRoot.Should().NotBeNull();

        ResultWrapper<ulong> finalHeadResult = await chain.ArbitrumRpcModule.HeadMessageIndex();
