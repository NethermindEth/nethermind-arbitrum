...
public static class ArbOwner
{
    ...
    private const ulong NativeTokenEnableDelay = 7 * 24 * 60 * 60; // 1 week in seconds
    ...
    public static void SetCompressionLevel(ArbitrumPrecompileExecutionContext context, ulong level)
    {
    ...
    {
        if (inkPrice > StylusParams.MaxInkPrice || inkPrice == 0)
        {
    ...
    public static void SetWasmMinInitGas(ArbitrumPrecompileExecutionContext context, ulong gas, ulong cached)
    {
        ...
        byte minCachedInitGas = byte.CreateSaturating(Utils.DivCeiling(gas, StylusParams.MinCachedGasUnits));
