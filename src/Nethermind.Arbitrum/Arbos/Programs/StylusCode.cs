...
public static class StylusCode
{
    ...
    public static OperationResult<StylusBytes> StripStylusPrefix(ReadOnlySpan<byte> code)
    {
        if (!IsStylusProgram(code))
            return OperationResult<StylusBytes>.Failure(OperationResultType.StylusInvalidByteCode, "Specified bytecode is not a Stylus program");

        BrotliCompression.Dictionary dictionary = (BrotliCompression.Dictionary)code[3];
        if (!Enum.IsDefined(dictionary))
            return OperationResult<StylusBytes>.Failure(OperationResultType.UnsupportedStylusCompressionDict, $"Unsupported Stylus dictionary {dictionary}");
